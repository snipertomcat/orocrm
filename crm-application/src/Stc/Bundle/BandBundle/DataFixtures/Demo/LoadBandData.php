...
namespace Stc\Bundle\BandBundle\DataFixtures\Demo;
...
/**
 * Loads fixtures bands with random data.
 */
class LoadTaskData extends AbstractFixture implements OrderedFixtureInterface
{
     ...
     * @var array
     */
    static protected $fixturesNames = array(
    ...
    public function load(ObjectManager $manager)
    {
        for ($i = 0; $i < self::FIXTURES_COUNT; ++$i) {
            ...
            $status = $this->getRandomEntity('StcBandBundle:BandStatus', $manager);
            ...
            if ($manager->getRepository('StcBandBundle:Band')->findOneByTitle(self::$fixtureTitles[$i])) {
                // Task with this title is already exist
                continue;
            }

            $task = new Band();
            $task->setName(self::$fixturesNames[$i]);
